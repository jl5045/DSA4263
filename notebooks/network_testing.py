...
# Assuming SplitMap = Dict[str, pd.DataFrame]
SplitMap = Dict[str, pd.DataFrame]

def run_network_features_only(
    split_frames: SplitMap,
    *,
    export: bool = False,          # default OFF for quick tests
    plots_dir: str | None = None,
    **add_net_kwargs,              # forwarded to add_network_features
) -> SplitMap:
    """
    Apply ONLY `add_network_features` across all splits in `split_frames`.
    Leaves the dict in-place and also returns it.
    """
    # apply_to_splits should call func(df, split_name, **kwargs)
    apply_to_splits(
        split_frames,
        add_network_features,
        **({"plots_dir": plots_dir} if plots_dir else {}),
        **add_net_kwargs
    )

    # Optional: quick schema sanity check to ensure features were added
    required_cols = [
        "sender_btwn","receiver_btwn",
        "sender_outdeg_amt","sender_indeg_amt",
        "receiver_outdeg_amt","receiver_indeg_amt",
    ...
    for split, df in split_frames.items():
        ...
        if missing:
            raise AssertionError(f"[{split}] missing expected network feature columns: {missing}")

    # No export by default (tests). If you really want, hook your existing exporter here.
    if export:
...
def main():
    ...
    print("\\nStarting feature engineering pipeline...")
    run_network_features_only(split_frames, export=True, plots_dir=plots_dir)
    print("Feature engineering pipeline complete.")
